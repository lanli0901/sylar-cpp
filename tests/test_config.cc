...
void print_yaml(const YAML::Node& node, int level){
    // 如果node是标量值
    // node.Scalar()：返回当前节点的标量值，通常是字符串、数字等基础类型的值 
    // node.Type()：返回节点的类型
    if(node.IsScalar()) {
...
void test_yaml(){
    // 加载YAML文件
    YAML::Node root = YAML::LoadFile("/home/lanli/learning/sylar_self/sylar-cpp/bin/conf/test.yml");
    print_yaml(root, 0);
...
sylar::ConfigVar<std::map<std::string, std::vector<Person> > >::ptr g_vec_person_map = 
    sylar::Config::Lookup("class.vec_map", std::map<std::string, std::vector<Person> >(), "class vec_map");
...
#define XX_PM(g_var, prefix) \
    ...
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "before: " << g_vec_person_map->toString();
    ...
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "after: " << g_vec_person_map->toString();
...
void test_log(){
    ...
    YAML::Node root = YAML::LoadFile("/home/lanli/learning/sylar_self/sylar-cpp/bin/conf/log.yml");
    sylar::Config::LoadFromYaml(root);
    std::cout << "=================================================" << std::endl;
    std::cout << sylar::LoggerMgr::GetInstance()->toYamlString() << std::endl;
    std::cout << "=================================================" << std::endl;
    ...
    SYLAR_LOG_INFO(system_log) << "hello systrm" << std::endl;
}
